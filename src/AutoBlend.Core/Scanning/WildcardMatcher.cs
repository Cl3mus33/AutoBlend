...
public static class WildcardMatcher
{
    ...
    // change during a run — but MatchesAny is called once per record/shape scanned, which on a
    // real modlist is tens of thousands of calls. RegexOptions.Compiled does real IL-generation/JIT
    // work per construction, so building a fresh Regex on every call (as before) meant redoing that
    // work for the exact same pattern string over and over. Caching the compiled Regex per
    // normalized pattern means each unique pattern is only ever compiled once per process; behavior
    // is identical since BuildRegex is a pure function of the normalized pattern.
